...
export default function RoadQueueHistory({ tatHistory, transactions, filters }: Props) {
    ...
    const applyTat = () => {
        router.get('/operations/road-queue/history', buildQuery(filters, tatValues), { preserveState: true, preserveScroll: true });
    };
    ...
    const applyTx = () => {
        router.get('/operations/road-queue/history', buildQuery(filters, txValues), { preserveState: true, preserveScroll: true });
    };
    ...
    const txExportUrl = buildExportUrl('/operations/road-queue/history/export/transactions', {
        tx_container: filters.tx_container,
        tx_category: filters.tx_category,
    });
    ...
    return (
        <AppLayout>
            ...
            <PageHeader title="Road Queue — History" description="Shift TAT averages and containers that spent 1 hour or more in the road queue." />
            ...
            <div className="space-y-8">
                <section>
                    <div className="mb-3 flex flex-wrap items-start justify-between gap-2">
                        ...
                        <CsvExportButton url={tatExportUrl} filename="road_queue_tat_history.csv" />
                    </div>
                ...
                <section>
                    <div className="mb-3 flex flex-wrap items-start justify-between gap-2">
                        ...
                        <CsvExportButton url={txExportUrl} filename="road_queue_high_elapsed_transactions.csv" />
                    </div>
                    ...
                    <HighElapsedTransactionsTable
                        rows={transactions}
                        dateColumn={{ key: 'precheck_time', label: 'Precheck Time' }}
                        emptyMessage={
